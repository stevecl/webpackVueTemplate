<template>
  <div class="jishi_pagination">
    <ul>
      ...
      <template v-for="(idx, index) in paging.total">
        <!--总数大于 翻页数*2+1 时-->
        <template v-if="paging.total > paging.flip * 2 + 1">
          <template v-if="idx === 1 && paging.page > paging.flip">
            <li @click="selectPage(idx)" :key='index'>
              {{idx}}
            </li>
            <!--选中页>翻页数+1 时显示-->
            <li class="noShow" v-if="paging.page > paging.flip + 2" :key="index">
              ...
            </li>
          </template>
          <!--当前渲染页 <= 选中页 + 翻页数 && >= 选中页 - 翻页数 时显示-->
          <template v-else-if="idx <= paging.page + paging.flip && idx >= paging.page - paging.flip">
            <li :class="{'check': idx === paging.page}" @click="selectPage(idx)" :key="index">
              {{idx}}
            </li>
          </template>
          <template v-else-if=" idx === paging.total && paging.page < paging.total - paging.flip">
            <!--选中页<总页数-翻页数-1 时显示-->
            <li class="noShow" v-if="paging.page < paging.total - paging.flip - 1" :key="index">
              ...
            </li>
            <li @click="selectPage(idx)" :key="index">
              {{idx}}
