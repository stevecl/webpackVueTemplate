;(function () {
  ...
  //判断时间是否在选择范围内
  function judgeDate(date, min, max) {
    var result = true;
  ...
  function Schedule(opt) {
    var def = {},
      ...
      selectedDate = '',
      minDate = opt.min ? opt.min : '1970-1-1',
      maxDate = opt.max ? opt.max : '3000-12-28',
    ...
    var bindEvent = function () {
      el.addEventListener('click', function (e) {
        ...
        ;
        if (e.target.className.indexOf('currentDate') > -1 && e.target.className.indexOf('disabled-date') == -1) {
    ...
    var render = function () {
      ...
      for (var i = 0; i < total; i++) {
        if (i < startWeek) {
          ...
          judgeDate(nowDate, minDate, maxDate) == false && (addClass += ' disabled-date');
    ...
    this.setDate = function (set_opt) {
      curDate = set_opt.date ? new Date(set_opt.date) : new Date();
      year = curDate.getFullYear();
      month = curDate.getMonth();
      day = curDate.getDate();
      currentYear = curDate.getFullYear();
      ...
      currentDay = curDate.getDate();
      selectedDate = '';
      minDate = set_opt.min ? set_opt.min : '1970-1-1';
      maxDate = set_opt.max ? set_opt.max : '3000-12-28';
