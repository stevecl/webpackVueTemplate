<template>
    ...
    <div>
        <div id="select-stock-container" :style="{'top':po_top+'px', 'left':po_left+'px'}" v-show="isShow">
            <div class="stock">
                <div class="change-month">
                    <img src="../assets/images/stock-arrow.png" @click="prevMonth">
                    <img src="../assets/images/stock-arrow.png" @click="nextMonth">
                ...
                <div class="select-date-box">
                    ...
                    <ul v-for="(rows, index) in dateList" :key="index">
                        <li :class="{'not-select': col.attr === 'false', 'active': nowDate === col.date}"
        ...
        <div class="btn-box">
            <a @click="btnShow">显示</a>
            <a @click="btnHide">隐藏</a>
            <span>选择的日期：{{selectValue}}</span>
        </div>
    </div>
    <!--主要容器结束-->
</template>
...
export default {
    ...
    methods: {
        ...
        render() {
            vm.dateList = [];
            this.nowDate = new Date(vm.cur_year, vm.cur_month, vm.cur_date).format("yyyy-MM-dd");
        ...
        // 选择日期
        selectedDate(col) {
            if (col.attr === "false") return;
            vm.isShow = false;
            vm.selectValue = col.date;
        },
        btnShow() {
            vm.init({
                date: vm.selectValue
...
.btn-box{
    ...
    a{
        ...
        line-height: 30px;
        color: #fff;
        background-color: rgba(62, 129, 211, 0.7);
        border-radius: 5px;
        cursor: pointer;
        &:hover{
            background-color: rgba(62, 129, 211, 1);
